import os
from unittest import TestCase
from .testbench import (Testbench, StreamSourceMock, StreamSinkMock,
                        AXI4LMasterMock, AXI4LSlaveMock)

class TestTestbench(TestCase):

    def test_basic(self):
        tb = Testbench()
        tb.add_include(os.path.dirname(__file__) + '/../vhdl/vhdmmio_pkg.vhd')
        test_in = tb.add_input('test_in', 8)
        test_out = tb.add_output('test_out', 8)
        tb.add_body('test_out <= std_logic_vector(unsigned(test_in) + 1);')
        with tb:
            self.assertEqual(test_in.val, '00000000')
            self.assertEqual(int(test_in), 0)
            test_in.val = 33
            self.assertEqual(test_in.val, '00100001')
            self.assertEqual(int(test_in), 33)

            self.assertEqual(test_out.val, '00000000')
            self.assertEqual(int(test_out), 0)
            tb.clock()
            self.assertEqual(test_out.val, '00100010')
            self.assertEqual(int(test_out), 34)

    def test_streams(self):
        tb = Testbench()
        tb.add_include(os.path.dirname(__file__) + '/../vhdl/vhdmmio_pkg.vhd')
        test_source = StreamSourceMock(
            tb.add_input('source_valid'),
            tb.add_output('source_ready'),
            tb.add_input('source_data', 8))
        test_sink = StreamSinkMock(
            tb.add_output('sink_valid'),
            tb.add_input('sink_ready'),
            tb.add_output('sink_data', 8))
        tb.add_body('sink_data <= std_logic_vector(unsigned(source_data) + 1);')
        tb.add_body('sink_valid <= source_valid;')
        tb.add_body('source_ready <= sink_ready;')
        with tb:
            test_source.send(33)
            with self.assertRaises(TimeoutError):
                test_source.wait(10)
            value = []
            def handler(data):
                value.append(int(data))
            test_sink.handle(handler)
            test_source.wait(10)
            self.assertEqual(value, [34])

    def test_axi(self):
        tb = Testbench()
        tb.add_include(os.path.dirname(__file__) + '/../vhdl/vhdmmio_pkg.vhd')
        master = AXI4LMasterMock(tb, 'master')
        slave = AXI4LSlaveMock(tb, 'slave')
        tb.add_body('slave_req <= master_req;')
        tb.add_body('master_resp <= slave_resp;')
        with tb:
            slave.start()
            self.assertEqual(master.read_bits(0), 'X'*32)
            master.write(0, 33)
            self.assertEqual(master.read(0), 33)
