    ...
    def generate(self):
        ...
        def expand(tple, block):
            if block is not None:
                tple.apply_str_to_str(block, postprocess=False)
                if not block.strip():
                    block = None
            return block
        ...
        if self.behavior.bus.read is not None:
            ...
            if self.behavior.bus.read.deferring:
                tple['defer'] = 'r_defer'
            lookahead = expand(tple, self.behavior.read_lookahead)
            ...
            if self.behavior.bus.read.blocking:
                tple['block'] = 'r_block'
            normal = expand(tple, self.behavior.read)

            tple['resp_ready'] = 'r_req'
            both = expand(tple, self.behavior.read_request)
            ...
            if self.behavior.bus.read.deferring:
                ...
                if self.behavior.bus.read.blocking:
                    tple['block'] = 'r_block'
                deferred = expand(tple, self.behavior.read_response)
            else:
                deferred = None
        ...
        if self.behavior.bus.write is not None:
            ...
            if self.behavior.bus.write.deferring:
                tple['defer'] = 'w_defer'
            lookahead = expand(tple, self.behavior.write_lookahead)

            tple['ack'] = 'w_ack'
            tple['nack'] = 'w_nack'
            if self.behavior.bus.write.blocking:
                tple['block'] = 'w_block'
            normal = expand(tple, self.behavior.write)

            tple['resp_ready'] = 'w_req'
            both = expand(tple, self.behavior.write_request)
            ...
            if self.behavior.bus.write.deferring:
                ...
                if self.behavior.bus.write.blocking:
                    tple['block'] = 'w_block'
                deferred = expand(tple, self.behavior.write_response)
            else:
                deferred = None
